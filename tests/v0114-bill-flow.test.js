...
import { prepareData, recordBillPayment, cycleTotals, reassignPayment, billDisplayCycle } from '../src/lib/finance.js'
...
test('a correction can create a missing prior bill occurrence without changing its balance effect',()=>{
  const paid=recordBillPayment(source(),{cycleId:'oct',bankId:'bank',date:'2026-09-08',amount:10},now)
  const moved=reassignPayment(paid,paid.billPayments[0].id,'new:2026-09-06','bill',now)
  const cycle=moved.billCycles.find(item=>item.id===moved.billPayments[0].cycleId)
  assert.equal(cycle.dueDate,'2026-09-06')
  assert.equal(moved.accounts[0].balance,90)
})

test('the Bills page keeps a paid monthly occurrence visible until the next month',()=>{
  const paid=recordBillPayment(source(),{cycleId:'sep',bankId:'bank',date:'2026-09-08',amount:10},now)
  const bill=paid.bills[0]
  assert.equal(billDisplayCycle(paid,bill,new Date(2026,8,21,12)).dueDate,'2026-09-06')
  assert.equal(billDisplayCycle(paid,bill,new Date(2026,9,1,12)).dueDate,'2026-10-06')
})
...
test('Bills has one primary entry flow and keeps corrections in collapsed history',async()=>{
  ...
  assert.match(app,/create missing occurrence/)
  assert.match(app,/paid monthly bill stays visible through month-end/)
