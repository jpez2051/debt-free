...
test('bill rows clearly label the due amount and current occurrence differences',async()=>{const source=await readFile(new URL('../src/App.jsx',import.meta.url),'utf8'),output=config.plugins[0].transform(source,'/src/App.jsx');assert.match(output,/`Due \$\{money2\.format\(amount\)\}`/);assert.match(output,/Current occurrence/);assert.match(output,/const VERSION='0\.10\.0'/)})
