...
test('release provides a top jump button and persistent reviewed controls',async()=>{const source=await readFile(new URL('../src/App.jsx',import.meta.url),'utf8'),output=source,component=await readFile(new URL('../src/DataHealth.jsx',import.meta.url),'utf8'),healthSource=await readFile(new URL('../src/lib/dataHealth.js',import.meta.url),'utf8');assert.match(output,/View Data Health/);assert.match(output,/aria-controls="data-health"/);assert.match(output,/scrollIntoView/);assert.match(output,/<DataHealth data=\{data\} update=\{update\}/);assert.match(healthSource,/Keep both/);assert.match(component,/Reviewed findings/);assert.match(component,/Review this again/);assert.match(output,/const VERSION='0\.10\.4'/)})
