...
import { ArrowDown, ArrowUp, Banknote, BarChart3, Check, CreditCard, Home, Lightbulb, Pencil, Plus, ReceiptText, Sparkles, Target, Trash2, WalletCards, X } from 'lucide-react'
...
export default function App(){
  ...
  const removePurchase=t=>{
    if(!confirm(`Remove ${t.merchant} for ${money2.format(t.amount)}?${t.historical?'':' This also reverses its balance effect.'}`))return
    let accounts=data.accounts
    if(!t.historical){accounts=data.accounts.map(a=>a.id!==t.accountId?a:{...a,balance:a.type==='credit'?Math.max(0,Number(a.balance)-Number(t.amount)):Number(a.balance)+Number(t.amount)})}
    update({...data,accounts,transactions:data.transactions.filter(x=>x.id!==t.id)})
  }

  const removePayment=p=>{
    if(!confirm(`Remove the ${money2.format(p.amount)} payment to ${p.cardName}?${p.historical?'':' This also restores the bank and card balances.'}`))return
    let accounts=data.accounts
    if(!p.historical){accounts=data.accounts.map(a=>a.id===p.bankId?{...a,balance:Number(a.balance)+Number(p.amount)}:a.id===p.cardId?{...a,balance:Number(a.balance)+Number(p.amount)}:a)}
    update({...data,accounts,payments:data.payments.filter(x=>x.id!==p.id)})
  }

  const removeAccount=a=>{
    const txCount=data.transactions.filter(t=>t.accountId===a.id).length
    const paymentCount=data.payments.filter(p=>p.bankId===a.id||p.cardId===a.id).length
    if(txCount||paymentCount){alert(`This account has ${txCount} purchase(s) and ${paymentCount} payment(s) linked to it. Remove those records first so balances stay accurate.`);return}
    if(!confirm(`Remove ${a.name}?`))return
    update({...data,accounts:data.accounts.filter(x=>x.id!==a.id)})
  }
...
function ActionRow({left,right,sub,onRemove}){return <div className="row action-row"><div><strong>{left}</strong>{sub&&<small>{sub}</small>}</div><div className="row-tail"><b>{right}</b><button type="button" className="icon-button danger" aria-label={`Remove ${left}`} title="Remove" onClick={onRemove}><Trash2 size={14}/></button></div></div>}
