...
export default function PaymentAssignments({data,update}) {
  ...
  return <details className="payment-assignments"><summary>Review payment assignments ({data.payments.filter(p=>p.assignmentStatus==='unassigned').length} needing review)</summary>
    ...
    <p>Select the statement you want a payment counted toward—even when its minimum is already met. That additional amount appears above the minimum for that statement, not the next one. Choose a general card payment only when you do not want to link it to any statement.</p>
    <form onSubmit={submit}>
      ...
      {payments.map(p=>{const statement=statements.find(s=>s.id===p.statementId),previous=statements.find(s=>s.id===p.previousStatementId);return <label className="assignment-check reliable-row" key={p.id}>
        ...
        <span>{transactionDay(p)} · {money.format(p.amount)} · From {data.accounts.find(a=>a.id===p.bankId)?.name||p.bankName}<small>{p.assignmentStatus==='confirmed'&&statement?`Statement due ${statement.dueDate}`:p.assignmentStatus==='extra'?'General card payment · no statement selected':p.assignmentStatus==='general'?'Historical payment · no statement selected':'Needs review'}{p.assignmentStatus==='unassigned'&&previous?` · Previous unverified assignment: due ${previous.dueDate}`:''}</small></span>
      </label>})}
      <label>Count selected payments toward<select value={target} onChange={e=>setTarget(e.target.value)}><option value="">Needs review — decide later</option><option value="__extra__">General card payment — no statement selected</option><option value="__general__">Historical payment — no statement selected</option>{statements.map(s=><option key={s.id} value={s.id}>Statement due {s.dueDate} · {money.format(s.minimum)} minimum{s.supersededBy?' · Carried forward':''}</option>)}</select></label>
      <p>{chosen.length} selected · {amount}. {target==='__extra__'?'These payments stay outside statement totals and the above-minimum insight.':target==='__general__'?'These historical payments stay outside statement totals.':target?`These payments will count toward the statement due ${statements.find(s=>s.id===target)?.dueDate}, not a later statement. Amounts above its minimum appear in Payments above minimum.`:'These payments will remain flagged for review.'} No amounts, payment dates, or balances will change.</p>
      <button className="primary" type="submit" disabled={!chosen.length}>{target==='__extra__'?'Mark general payment':target==='__general__'?'Mark historical payment':target?'Confirm statement':'Keep for review'}</button>
