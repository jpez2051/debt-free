...
export default function PaymentAssignments({data,update}) {
  ...
  return <details className="payment-assignments"><summary>Review payment assignments ({data.payments.filter(p=>p.assignmentStatus==='unassigned').length} needing review)</summary>
    ...
    <p>Select payments for the same statement—even if you paid early or more than once. Mark intentional balance-reducing payments as extra/general, or historical/general when no statement assignment is needed.</p>
    <form onSubmit={submit}>
      ...
      {payments.map(p=>{const statement=statements.find(s=>s.id===p.statementId),previous=statements.find(s=>s.id===p.previousStatementId);return <label className="assignment-check reliable-row" key={p.id}>
        ...
        <span>{transactionDay(p)} · {money.format(p.amount)} · From {data.accounts.find(a=>a.id===p.bankId)?.name||p.bankName}<small>{p.assignmentStatus==='confirmed'&&statement?`Assigned: due ${statement.dueDate}`:p.assignmentStatus==='extra'?'Extra/general · no statement needed':p.assignmentStatus==='general'?'Historical/general · no statement needed':'Needs review'}{p.assignmentStatus==='unassigned'&&previous?` · Previous unverified assignment: due ${previous.dueDate}`:''}</small></span>
      </label>})}
      <label>Classify selected payments<select value={target} onChange={e=>setTarget(e.target.value)}><option value="">Needs review — decide later</option><option value="__extra__">Extra/general payment — no statement needed</option><option value="__general__">Historical/general — no statement needed</option>{statements.map(s=><option key={s.id} value={s.id}>Statement due {s.dueDate} · {money.format(s.minimum)} minimum{s.supersededBy?' · Carried forward':''}</option>)}</select></label>
      <p>{chosen.length} selected · {amount}. {target==='__extra__'?'These are intentional extra/general payments. They reduce card debt but do not count toward a statement minimum.':target==='__general__'?'These will remain in history without counting toward a statement or changing balances.':target?`These payments will count toward the statement due ${statements.find(s=>s.id===target)?.dueDate}.`:'These payments will remain flagged for review.'} No amounts, payment dates, or balances will change.</p>
      <button className="primary" type="submit" disabled={!chosen.length}>{target==='__extra__'?'Mark extra/general':target==='__general__'?'Mark historical/general':target?'Confirm assignment':'Keep for review'}</button>
