...
import { Cloud, Download, LogIn, LogOut, ShieldCheck, Upload } from 'lucide-react'
...
export default function CloudApp() {
  ...
  useEffect(() => {
    const stop = observeUser(async signedIn => {
      ...
      try {
        ...
        else { setPhase('migration'); setCloudSession({ status:'migration', user:signedIn, sync:'Ready to import', actions:{ signOut:logout, importLocal:importLocalData } }) }
  ...
  if (phase === 'migration') return <CloudScreen><span className="kicker">ONE-TIME MOVE</span><h1>Move this browser’s records to your private cloud.</h1><p>Your existing accounts, activity, bills, payments, and scores will be copied to the Google account you just selected. The browser copy stays in place until you choose to remove it later.</p><div className="cloud-migration-counts"><span>{localData.accounts.length} accounts</span><span>{localData.transactions.length} activity entries</span><span>{localData.payments.length+(localData.billPayments||[]).length} payments</span></div><button className="primary cloud-action" type="button" onClick={importLocalData}><Upload size={17}/> Copy my records to the cloud</button><button className="secondary cloud-action" type="button" onClick={logout}><LogOut size={17}/> Use a different Google account</button>{message&&<div className="backup-message">{message}</div>}<small><ShieldCheck size={14}/> Nothing is uploaded until you select “Copy my records to the cloud.”</small></CloudScreen>
