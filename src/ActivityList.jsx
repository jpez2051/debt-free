...
export default function ActivityList({data,renderTransaction,onStatements,onBills,onAccounts}) {
  ...
  return <section className="activity-list" aria-label="Activity history">
    ...
    <div className="table-list">{entries.map(row=>row.source==='transaction'?<Fragment key={row.id}>{renderTransaction(row.entry)}</Fragment>:<div className="activity-history-row" key={row.id}>
      <div><strong>{row.title}</strong><small>{new Date(`${row.date}T12:00:00`).toLocaleDateString()} · {row.detail}{row.entry.historical?' · Historical':''}</small>{row.source==='billPayment'&&<small>{row.type==='purchase'?'Recurring card charge':'Bill payment'}</small>}{row.source==='cardPayment'&&<small>{row.entry.assignmentStatus==='confirmed'?`Statement due ${data.cardStatements.find(s=>s.id===row.entry.statementId)?.dueDate||'unknown'}`:row.entry.assignmentStatus==='extra'?'Extra/general payment':row.entry.assignmentStatus==='general'?'Historical/general payment':'Statement not assigned'}</small>}</div>
