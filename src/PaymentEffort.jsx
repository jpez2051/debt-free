...
export default function PaymentEffort({data,now=new Date()}) {
  ...
  return <section className="panel content-panel payment-effort" aria-labelledby="payment-effort-title">
    ...
    {(pending>0||general>0)&&<div className="payment-effort-review"><p className="payment-effort-note">{pending>0?`${pending} payment${pending===1?'':'s'} still need statement review. `:''}{general>0?`${general} intentional extra/general payment${general===1?' is':'s are'} not tied to a statement. `:''}These are excluded from the above-minimum total; assign one to a statement only if it belongs to that billing cycle.</p><button type="button" className="secondary compact" onClick={()=>{const target=document.querySelector('.payment-assignments');if(target){target.open=true;target.scrollIntoView({behavior:'smooth',block:'start'})}}}>Review assignments</button></div>}
