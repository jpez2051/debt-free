...
export function transformAppV054(source) {
  ...
  code = replaceOrThrow(
    ...
    " const removePayment=p=>{if(!confirm(`Remove the ${money2.format(p.amount)} payment to ${p.cardName}?${p.historical?'':' This restores both balances and its billing-cycle status.'}`))return;if(!p.historical){const index=data.payments.findIndex(x=>x.id===p.id),newer=index>0&&data.payments.slice(0,index).some(x=>!x.historical&&x.cardId===p.cardId);if(newer){alert('Remove newer current payments for this card first so the billing-cycle history stays accurate.');return}}let accounts=data.accounts;if(!p.historical)accounts=accounts.map(a=>a.id===p.bankId?{...a,balance:Number(a.balance)+Number(p.amount)}:a.id===p.cardId?{...a,balance:Number(a.balance)+Number(p.amount),nextDueDate:p.cycleDueDateBefore||a.nextDueDate,cyclePaidAmount:Number(p.cyclePaidBefore||0),lastPaidDueDate:p.lastPaidDueDateBefore||'',lastCyclePaidDate:p.lastCyclePaidDateBefore||''}:a);update({...data,accounts,payments:data.payments.filter(x=>x.id!==p.id)})}",
    'payment reversal'
  )
